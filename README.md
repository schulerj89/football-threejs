...
- Custom title settings expose gameplay camera, cinematics, visual crowd, stadium, crowd density, crowd reactions, master audio, crowd audio, announcer, captions, and quality mode.
...
- A plain launch uses the `broadcast` experience preset: 11v11 playbook, offense camera, brief cinematics, mathematical stadium, low-density crowd visuals and reactions, runtime audio, announcer, pre-snap route art, and player motion.
...
- Query parameters such as `?camera=`, `?cinematics=`, `?stadium=`, `?crowdVisuals=`, `?audio=`, `?announcer=`, `?captions=`, `?routeArt=`, and `?playerMotion=` are explicit development overrides and do not rewrite persisted experience settings.
