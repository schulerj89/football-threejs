...
- A normal launch opens the title screen. Choose `Start Game` to enter the selected prototype and unlock browser audio from that user gesture. Exhibition starts run the pregame broadcast presentation, including the user-team starting quarterback spotlight when cinematics are enabled, before playable pre-snap; `Space` or `Enter` skips the intro without snapping the ball.
