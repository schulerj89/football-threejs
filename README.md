...
- Scenario: 11v11 broadcast profile, offense camera, brief cinematics, procedural player motion, presentation-only officials, full crowd fullness with 500 reacting near spectators and 4,500 static mosaic seats, route art enabled, debug overlays disabled.
...
- Current structural budgets are measured from the passing optimized reference plus margin: `5000` visible crowd seats, `450` draw calls, `250000` triangles, `180` geometries, `90` materials, `32` textures, `390` visible player meshes, `0` shadow casters, `8` crowd draw calls, and `20` stadium draw-call estimate.
...
- `Apply Recommended Count` maps the session recommendation to the compatible crowd fullness/density setting; crowd visuals are not automatically enabled.
...
- Generated starter pack: two streamed crowd loops, six crowd reactions, seven football one-shots, three stadium chant layers, 27 gameplay announcer speech clips, and 49 pregame broadcast clips.
...
The richer report can be printed with `npm run audio:report` and written with `npx tsx tools/audio/audioReport.ts --write`. `npm run audio:verify` validates runtime-manifest filenames, non-empty decodable MP3s, duration bounds, provenance sidecars, caption metadata, compressed-size budget, decoded-buffer budget policy, readiness classification, and the audition page at `public/audio/audition-index.html`. The browser runtime mixer consumes local files only. Runtime announcer playback uses the local `public/audio/announcer/*.mp3` and `public/audio/announcer/pregame/*.mp3` files, serializes commentary so clips do not overlap, and shows exact captions when enabled. The title, Match Setup, and Settings screens stream the local menu playlist through the runtime music bus after a user gesture, show a compact now-playing indicator, and preserve the current track while moving between menu screens. Menu music ducks and fades through the pregame handoff and stops before playable pre-snap. During gameplay, the runtime uses only short transition stingers and safe-phase crowd chants; it does not play a continuous in-game soundtrack. Spatial player sounds, live text generation, second-commentator support, additional pregame segments beyond warmups/QB showcase, and dynamic gameplay music are queued future tasks.

`npm run music:generate` defaults to dry-run. Paid ElevenLabs Music and chant-generation calls require direct execution with `--execute`, for example `npx tsx tools/audio/generateMusic.ts --execute --max-files=15`. Existing audio or sidecar files are skipped unless `--force` is supplied. The current generated title-theme candidates are `public/audio/music/football-js-title-a.mp3`, `football-js-title-b.mp3`, and `football-js-title-c.mp3`; candidate A is copied to the stable provisional path `public/audio/music/football-js-title.mp3`. The expanded pack adds three menu playlist tracks, six transition stingers, and three crowd chant layers under `public/audio/music` and `public/audio/crowd/chants`. The grouped audition page is `public/audio/music/music-audition.html`, the runtime catalog is `public/audio/music/music-catalog.json`, the report is `public/audio/music/music-report.json`, and the selection manifest is `public/audio/music/music-selection.json`. Runtime menu playback consumes the stable TypeScript catalog in `src/audio/MusicCatalog.ts`; browser code still never calls ElevenLabs.
...
- Custom title settings expose quarter length, difficulty, gameplay camera, cinematics, visual crowd, stadium, crowd fullness, crowd reactions, sideline teams, head coaches, tunnel tableau, officials, master audio, crowd audio, announcer, captions, quality mode, and team/uniform customization.
- Custom title settings expose music enablement, music volume, and sequential/shuffle menu playlist order. The now-playing indicator provides small previous/next controls without adding another primary title button.
...
- In the default 11v11 prototype, press `3` during pre-snap to select `Outside Zone 11`.
- In the default 11v11 prototype, press `4` during pre-snap to select `Off Tackle 11`.
- In the default 11v11 prototype, press `5` during pre-snap to select `Twin Slants 11`.
- In the default 11v11 prototype, press `6` during pre-snap to select `Curl Flat 11`.
...
- A plain launch uses the `broadcast` experience preset with the offense-only Exhibition game mode: 11v11 playbook, offense camera, brief pregame/cinematic presentation, mathematical stadium, presentation-only officials, sideline reserve teams, head coaches, tunnel tableau staging, full crowd visuals with a bounded reacting lower bowl and static far-bowl mosaic, runtime audio, announcer, pre-snap route art, and player motion.
...
- Query parameters such as `?camera=`, `?cinematics=`, `?stadium=`, `?officials=`, `?sidelinePlayers=`, `?sidelineDensity=`, `?coaches=`, `?tunnelTableau=`, `?crowdVisuals=`, `?audio=`, `?announcer=`, `?captions=`, `?routeArt=`, and `?playerMotion=` are explicit development overrides and do not rewrite persisted experience settings.
...
- Use `?sidelinePlayers=0` to disable presentation-only sideline reserve players, `?sidelineDensity=low|medium|high` to choose 5/10/15 reserve players per team, `?coaches=0` to disable presentation-only head coaches, and `?tunnelTableau=0` to hide the current static tunnel tableau. Use `?sidelineDebug=1` or the F1 debug panel's `Sideline teams` feature to inspect counts, coach state, zones, draw calls, triangles, instance bytes, and update cadence.
...
- The broadcast preset enables the mathematical stadium and the normal-game budgeted visual crowd at full visual fullness. Use `?stadium=0` to disable stadium geometry for a session. Use `?crowdVisuals=0` to disable the visual crowd, or `?crowdVisuals=1` to explicitly enable it when using another preset. Use `?crowdFullness=sparse`, `?crowdFullness=standard`, or `?crowdFullness=full` for measured visual fullness profiles. Legacy `?crowdDensity=low|medium|high` aliases remain supported for deep links. Use `?crowdReactions=0` to keep the reacting near tier static.
- The graphical play cards are visible only during pre-snap and are generated from the same play definitions, resolved formation positions, ordered receiver routes, blocking assignments, pass-protection assignments, snap lane, and field/boundary side used by gameplay. Route diagrams show every declared segment and break point rather than reducing routes to a final target.
...
- Coverage defenders track their assigned receivers while ordinary defenders use the existing simple pursuit or pass-rush behavior. In the default 11v11 playbook, Inside Zone 11, Outside Zone 11, and Off Tackle 11 use explicit one-to-one run-blocking assignments, while Spread Quick 11, Twin Slants 11, and Curl Flat 11 use five offensive pass protectors, five eligible receivers, explicit coverage assignments, and deep safety help. Defenders switch to carrier pursuit after a catch.
...
- Use `?routeArt=0` to disable pre-snap on-field receiver route art, or `?routeArt=1` to explicitly enable offense-only receiver route art.
- Use `?coverageArt=1` or `?defenseArt=1` to show defense-only pre-snap coverage-zone art. Use `?bothPlayArt=1` only for deliberate offense/defense overlay comparisons.
...
Use the F1 debug panel or `?sidelineDebug=1` to show the sideline teams overlay. It reports active reserve, coach, and tunnel counts, density, coach state, reaction state, draw calls, triangles, instance-buffer bytes, update frequency, team zones, tunnel zones, and visible mesh count.
...
Add `?crowdDebug=1` with `?crowdVisuals=1` to show the normal-game crowd presentation overlay. It reports visual/reaction settings, crowd fullness, spectator count, near reacting count, far mosaic count, reaction state, bounded update count, draw calls, triangles, frame time, and the latest crowd event sync.
...
- Presentation future scope: detailed stadium architecture, detailed crowd choreography, assistant coaches, chain crew, advertisements, weather, field degradation, turf redesign, animated scoreboard video, and broader stadium polish are planned later, not permanent exclusions. The current presentation work includes an active mathematical stadium bowl, optional normal-game visual crowd presentation, presentation-only sideline reserve teams, head coaches, and tunnel staging, plus the development-only rendering and memory preview.
- Roster future scope: 11v11 is now the active normal prototype with three rushing plays and three passing plays. The current roster layer provides fictional starter identities, specialists, setup preview, and active-lineup presentation binding. Additional 11v11 plays, full special teams behavior, depth-chart editing, substitutions, injuries, recruiting, roster tiers beyond the current 11v11 prototype and maintained 7v7/5v5 regression modes, player switching, and broader formation families are deferred.
