...
- The title screen supports `Broadcast`, `Performance`, and `Custom` presentation profiles plus `7v7 Prototype` and `5v5 Legacy Development Mode` game modes.
...
- In the default 7v7 prototype, press `1` during pre-snap to select `Inside Zone 7`.
- In the default 7v7 prototype, press `2` during pre-snap to select `Outside Zone 7`.
- In the default 7v7 prototype, press `3` during pre-snap to select `Quick Pass 7`.
- In the default 7v7 prototype, press `4` during pre-snap to select `Twin Slants Flat`.
- Use `?playbook=7v7` to explicitly launch the default 7v7 playbook, use `?playbook=5v5` / `5v5 Legacy Development Mode` to run the preserved five-on-five regression drill, or use `?playbook=11v11` to launch the optional one-play Inside Zone 11 development drill.
...
- A plain launch uses the `broadcast` experience preset: 7v7 playbook, offense camera, brief cinematics, low-density crowd visuals and reactions, runtime audio, announcer, pre-snap route art, and player motion.
...
- Coverage defenders track their assigned receivers while ordinary defenders use the existing simple pursuit or pass-rush behavior. In the default 7v7 playbook, the center and line players use explicit run-blocking or pass-protection assignments, corners and linebacker use explicit coverage assignments on passing plays, and defenders switch to carrier pursuit after a catch.
...
- Roster future scope: 11v11 is currently staged only as a static development formation preview. Active 11v11 gameplay, full special teams, additional roster tiers beyond the current default 7v7 playbook and legacy 5v5 regression mode, player switching, and broader formation families are deferred.
