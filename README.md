...
- The title screen supports `Broadcast`, `Performance`, and `Custom` presentation profiles plus `5v5 Prototype` and `7v7 Passing Prototype (limited)` game modes.
...
- Select `7v7 Passing Prototype (limited)` on the title screen, or use `?playbook=7v7`, to launch the optional seven-on-seven `Twin Slants Flat` passing drill. In that mode, press `1` during pre-snap to select the available 7v7 play.
...
- A plain launch uses the `broadcast` experience preset: 5v5 playbook, offense camera, brief cinematics, low-density crowd visuals and reactions, runtime audio, announcer, pre-snap route art, and player motion.
...
- Audio future scope: no spatial player sounds, music playback, live text generation, second commentator, or browser-side ElevenLabs calls in runtime gameplay. Starter-pack MP3 playback depends on generated local files being present.
