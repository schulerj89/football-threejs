...
The current milestone adds optional cinematic orbit-shot presentation for pre-play setup and touchdown results while keeping snap input responsive and preserving every existing gameplay camera. Crowd presentation is planned product scope, but the current crowd renderer remains disabled in normal gameplay unless `?crowdPreview=1` is used. ElevenLabs remains offline-only production tooling with no browser API calls. The default playable prototype remains a two-minute five-on-five offensive score-attack drill with semantic data-defined formations for Inside Run, Outside Run, Quick Pass, and Slant Flat, and `?playbook=7v7` runs the optional Twin Slants Flat passing drill. The prototype includes runtime broadcast-announcer playback from local pre-rendered assets, graphical pre-snap SVG play cards generated from gameplay data, visual-only procedural player poses and locomotion, route-aware passing consistency, receiver routes as ordered mathematical paths, pre-snap field route art, development-only route/pass/appearance/audio/commentary/crowd/camera auditing, a procedural football, visible low-poly player heads, and a basic offensive drive: a field generated from a pure field specification with batched static markings, turf bands, yard numbers, goalposts, sideline presentation, selectable play calls, quarterback scrambling with a line-of-scrimmage passing rule, route-running receiver behavior, selected-target passing with a deterministic arc, swept catch detection, downs, yards-to-go, first-down line, touchdown scoring, sack, tackle, incomplete, and out-of-bounds outcomes, turnover-on-downs reset, exact dead-ball spotting with three-lane snap placement, final-score game over, a development-only `?formationPreview=7v7` staging mode, the preserved orthographic three-quarter camera, the behind-the-offense perspective camera, and the optional cinematic broadcast camera.
...
- First starter-pack plan: two streamed crowd loops, six crowd reactions, seven football one-shots, and 27 announcer speech scripts.
...
The richer report can be printed with `npm run audio:report` and written with `npx tsx tools/audio/audioReport.ts --write`. The browser runtime mixer consumes local files only. The starter-pack MP3s are optional at runtime: missing files warn in development and do not stop gameplay. Runtime announcer playback uses the local `public/audio/announcer/*.mp3` files when present, serializes commentary so clips do not overlap, and shows exact captions when enabled. Generated asset approval/import, spatial player sounds, live text generation, and music are queued future tasks.
...
- Use `?cinematics=off`, `?cinematics=brief`, or `?cinematics=full` to control optional orbit-shot presentation. The default is `off`.
- Use `?shotPreview=prePlayOrbit180` or `?shotPreview=touchdownOrbit360` with `?cameraDebug=1` to inspect either orbit shot without changing gameplay rules.
...
Add `?cameraDebug=1` to show camera debug output, including active orbit shot name, shot progress, orbit center, radius, camera position, look target, and restore camera. `?cinematics=brief` and `?cinematics=full` enable the optional `prePlayOrbit180` and `touchdownOrbit360` presentation shots; `?cinematics=off` disables them. Shot previews can be forced with `?shotPreview=prePlayOrbit180` or `?shotPreview=touchdownOrbit360`.
...
- Presentation future scope: normal-game stadium activation, reactive crowd presentation, cinematic presentation polish, sideline characters, advertisements, weather, field degradation, turf redesign, and broader stadium presentation are planned later, not permanent exclusions. The current crowd work is a development-only rendering and memory preview.
...
- Audio future scope: no spatial player sounds, music playback, live text generation, second commentator, or browser-side ElevenLabs calls in the current milestone. Starter-pack MP3 playback depends on generated local files being present.
