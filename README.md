...
- A plain launch uses the `broadcast` experience preset with the offense-only Exhibition game mode: 11v11 playbook, offense camera, brief cinematics, mathematical stadium, presentation-only officials, sideline teams, tunnel tableau staging, low-density crowd visuals and reactions, runtime audio, announcer, pre-snap route art, and player motion.
