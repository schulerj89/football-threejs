...
- Custom title settings expose gameplay camera, cinematics, visual crowd, stadium, crowd density, crowd reactions, officials, master audio, crowd audio, announcer, captions, quality mode, and team/uniform customization.
...
- A plain launch uses the `broadcast` experience preset: 11v11 playbook, offense camera, brief cinematics, mathematical stadium, presentation-only officials, low-density crowd visuals and reactions, runtime audio, announcer, pre-snap route art, and player motion.
...
- Play calling and setup: no large playbook menu, final title artwork, real team logos, audibles, defensive play selection, route editor, procedural play generation, hot routes, roster editing, season mode, save slots, or menus beyond the current title/setup flow, team customization panel, pause settings panel, pre-snap play cards, and minimal HUD/debug displays.
