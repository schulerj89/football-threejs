...
import { FOOTBALL_EXPANDED_MUSIC_PLAN } from './musicPlan';
import {
  assetOutputExists,
  assertCanWriteAsset,
...
const MAX_EXPANDED_MUSIC_ASSETS_PER_EXECUTION = 15;
...
export interface MusicGenerationDependencies {
  ...
  requestSoundEffect?: (asset: AudioAssetPlan, apiKey: string) => Promise<MusicGenerationResult>;
}
...
export async function generateMusic(
  ...
  if (options.maxFiles > MAX_EXPANDED_MUSIC_ASSETS_PER_EXECUTION) {
    throw new Error(
      `Expanded music generation is capped at ${MAX_EXPANDED_MUSIC_ASSETS_PER_EXECUTION} assets per execution.`,
    );
  }

  const selectedAssets = plan
    .filter(isExpandedMusicPackAsset)
  ...
  const requestSoundEffect = dependencies.requestSoundEffect ?? requestElevenLabsSoundEffect;
  const generated: string[] = [];

  for (const asset of assetsToGenerate) {
    const result = await withSingleRetry(options.retryCount, () =>
      asset.category === 'music'
        ? requestMusic(asset, apiKey)
        : requestSoundEffect(asset, apiKey),
    );
...
async function requestElevenLabsMusic(asset: AudioAssetPlan, apiKey: string): Promise<MusicGenerationResult> {
  ...
  const response = await fetch(url, {
    body: JSON.stringify({
      ...
      music_length_ms: Math.round(asset.requestedDurationSeconds * 1000),
      prompt: asset.prompt ?? '',
    }),
    headers: {
...
async function requestElevenLabsSoundEffect(asset: AudioAssetPlan, apiKey: string): Promise<MusicGenerationResult> {
  const url = new URL('https://api.elevenlabs.io/v1/sound-generation');
  url.searchParams.set('output_format', asset.outputFormat);
  const response = await fetch(url, {
    body: JSON.stringify({
      duration_seconds: asset.requestedDurationSeconds,
      loop: asset.loop,
      model_id: asset.modelId,
  ...
  if (!response.ok) {
    const errorBody = await response.text();
    throw new Error(`ElevenLabs sound-effect generation failed for ${asset.assetId}: ${response.status} ${errorBody}`);
  }
...
function isExpandedMusicPackAsset(asset: AudioAssetPlan): boolean {
  return asset.category === 'music' || asset.metadata?.catalogCategory === 'chant';
}
...
if (isDirectCli(import.meta.url)) {
  ...
  generateMusic(FOOTBALL_EXPANDED_MUSIC_PLAN, options)
    .then((summary) => {
    ...
    .catch((error: unknown) => {
      ...
      if (error instanceof Error && /Missing ELEVENLABS_API_KEY/.test(error.message)) {
        console.log(JSON.stringify({
          dryRunPlan: FOOTBALL_EXPANDED_MUSIC_PLAN,
          validationErrors: [],
        }, null, 2));
      }
      process.exitCode = 1;
