...
- Every active lineup slot resolves to one stable fictional roster identity without changing gameplay player IDs.
- Jersey numbers are unique within each starter roster, and invalid position-number conventions remain warnings instead of hard failures.
- The setup screen shows a lightweight roster preview with number, name, and football position for the selected mode and teams.
- The controlled-player label starts on the correct carrier, transfers exactly once after a catch, resets to the correct identity, and caches textures instead of redrawing every frame.
- Label settings, debug readback, README, AGENTS, package version, and CHANGELOG reflect the roster-identity and retro-label release.
