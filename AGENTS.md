...
- Audio future scope: no spatial player sounds, music playback, live text generation, conversational agents, second commentator, or browser-side ElevenLabs calls in runtime gameplay. Starter-pack MP3 playback depends on generated local files being present.
...
- The broadcast experience preset is the default normal-game profile: 5v5 playbook, offense camera, brief cinematics, low-density crowd visuals, crowd reactions, runtime audio, crowd audio, announcer enabled, captions disabled, route art enabled, and player motion enabled.
...
- Normal launches show a title screen before gameplay begins.
- Start Game applies the selected presentation preset and game mode, unlocks or requests browser audio unlock from the user gesture, enters pre-snap, and reveals HUD/play cards.
- The title screen offers Broadcast, Performance, and Custom presentation choices plus 5v5 Prototype and limited 7v7 Passing Prototype game modes.
- Custom settings expose gameplay camera, cinematics, visual crowd, crowd density, crowd reactions, master audio, crowd audio, announcer, and captions.
- Gameplay clock, AI, play controls, play cards, and presentation events do not advance behind the title screen.
- Escape opens a small settings panel during pre-snap or dead-ball states, and live play remains uninterrupted.
- Settings persist across reloads, while query overrides and development preview/audit modes remain opt-in and direct.
- Missing optional audio stays non-blocking and non-player-facing.
