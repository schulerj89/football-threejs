...
- The current runtime audio manifest is populated with real local MP3 files for the planned crowd loops, crowd reactions, football one-shots, and prototype announcer catalog.
- Every generated asset has a provenance sidecar with prompt or script, model, voice ID where applicable, generation date, output format, duration, byte count, and content hash.
- `npm run audio:verify` reports readiness `complete`, no missing runtime assets, no caption mismatches, no manifest filename mismatches, and no provenance errors.
- The audition page at `public/audio/audition-index.html` lists crowd assets, effects, announcer lines, captions/prompts, duration, size, and regeneration metadata.
- Runtime audio unlock, mute, crowd streaming, one-shot decoding, and announcer playback continue to use local files only.
- No ElevenLabs API key or request header appears in browser source, generated manifests, or committed files.
