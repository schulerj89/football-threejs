...
- The default ball visual is a recognizable procedural low-poly football with longer axis than width, seam, laces, shared resources, carried orientation, and in-flight visual alignment.
- `?ballVisual=sphere` preserves comparison with the previous round placeholder ball.
- Every active player has a low-poly head and neck under the existing head anchor, visible through the helmet faceguard without changing helmet GLB loading or tinting.
- Skin tones are deterministic from stable player ID, shared by palette material, and visibly varied across the default roster.
- `?appearanceAudit=1` reports player ID, skin-tone ID, head bounds, helmet bounds, and head-to-helmet clearance.
- Ball and player-head presentation does not change gameplay snapshots, ball physics, player collision, AI, passing rules, or possession.
