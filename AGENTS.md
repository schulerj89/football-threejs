...
- Pass targeting predicts the selected receiver from route progress, route speed, pass speed, and bounded deterministic iteration.
- Normal route-aware passing no longer uses a fixed lead-distance fallback.
- Catch detection is robust to the ball crossing the catch region between simulation frames.
- `?passAudit=1` shows selected receiver, release position, predicted target, predicted receiver position, predicted flight time, closest approach, miss distance, catch height, and result reason.
- Equivalent passing scenarios at common update rates produce the same catch or incompletion result.
- Existing five-on-five gameplay, optional 7v7 passing drill, route art, play cards, helmet, visual, camera, unit, and browser smoke tests pass.
