...
- Plain launch, broadcast preset, performance preset, fallback playbook resolution, and the title/setup default select the 7v7 playbook.
- The default 7v7 playbook contains Inside Zone 7, Outside Zone 7, Quick Pass 7, and Twin Slants Flat, all resolving cleanly at left-hash, middle, and right-hash snap lanes with seven offensive and seven defensive players.
- 5v5 remains available only through explicit legacy development/regression selection and its existing gameplay tests continue passing.
- Number keys `1` through `4`, graphical play cards, reset behavior, receiver cycling, route art, and possession rules all work with the active 7v7 play data.
- README, AGENTS, package version, and CHANGELOG reflect the 7v7 default promotion and 11v11 next roster milestone.
