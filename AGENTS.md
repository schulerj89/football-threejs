...
- Runtime audio must create exactly one `AudioContext`, route crowd, announcer, gameplay SFX, and UI buses through master, and start or resume only after a user gesture.
...
- `tools/audio/audioPlan.ts` contains the requested crowd loops, crowd reactions, pad hits, catches, whistle, and snap assets using `eleven_text_to_sound_v2` and `mp3_44100_128`.
- Crowd loops are marked as streamed assets; one-shots are marked as decoded-buffer candidates.
- Dry-run planning validates without an API key and does not create audio files.
- Paid generation requires a local `ELEVENLABS_API_KEY` and skips existing successful files unless `--force` is supplied.
- Audio reporting records compressed size, duration when files exist, one-shot decoded-memory estimates, provenance sidecar status, and starter-pack budget status without decoding crowd loops.
- If the key is missing or invalid, stop at the validated dry-run plan and do not fabricate MP3s, sidecars, or approved outputs.
- Generated audio integration, announcer playback, crowd reactions, camera changes, and gameplay changes are not implemented in this milestone.
