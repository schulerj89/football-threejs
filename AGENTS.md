...
- Presentation audio events cover play preparation, snap, catch, tackle, incomplete, out-of-bounds, first down, touchdown, sack, turnover on downs, and reset where supported.
- Each gameplay result emits its mapped audio event once, and reset/dead-state rerenders do not retrigger the previous result.
- Crowd ambience targets idle during pre-snap, pressure during live play, and returns to idle after results while keeping at most two loop instances active.
- First downs, touchdowns, incompletions, turnovers, snaps, catches, tackles, sacks, and whistle cooldowns map to deterministic local asset IDs.
- Muted/audio-disabled modes do not load or play event assets, and missing optional pack files do not stop gameplay.
- `?audioDebug=1` shows event history with event ID, chosen asset, trigger time, status, and suppression reason.
