...
- Runtime audio modules exist for mixer, asset loading, settings, gameplay-event observation, and the game audio director.
- Exactly one `AudioContext` is created, buses route through master, and playback unlocks from a user gesture without repeated console failures.
- Short one-shot assets use decoded `AudioBuffer`s and long loop assets use streamed media elements.
- Settings persist locally, `M` toggles mute, and `?audio=0`, `?crowdAudio=0`, `?announcer=0`, and `?audioDebug=1` work.
- Missing optional local audio files warn in development without blocking scene startup.
- No ElevenLabs API key or SDK code appears in browser-facing source or the browser bundle.
...
- Existing five-on-five gameplay, optional 7v7 passing drill, route art, play cards, helmet, mannequin, visual, camera, unit, and browser smoke tests pass.
