...
This repository is a low-poly 3D American football game prototype built with Three.js, Vite, TypeScript, WebGL, and a future WebGPU path. The long-term target is a stylized low-poly 11v11 American-football game with cinematic and broadcast-style presentation. The current score-attack mode is a temporary gameplay test harness, not the final product identity. The active prototype is the fourteen-player 7v7 default game mode while 5v5 remains an explicit legacy development and regression mode. The 11v11 roster is currently staged only through static development formation preview, not active gameplay. Current presentation integration work should preserve one authoritative immutable presentation-event stream for audio, commentary, crowd, camera, captions, and holds.
...
- Roster future scope: 11v11 is currently static formation staging only. Active 11v11 gameplay, full special teams, additional roster tiers beyond the current default 7v7 playbook and legacy 5v5 regression mode, player switching, and broader formation families are deferred.
