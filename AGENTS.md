...
- Controls and camera: no sprinting, stamina, freely rotating camera, camera-relative controls, instant replay, replay recording, slow motion, fan cutaways, camera collision, or camera redesign beyond the current tactical, offense-perspective, cinematic broadcast, and optional orbit-shot presentation modes.
- Audio future scope: no spatial player sounds, music playback, live text generation, conversational agents, second commentator, or browser-side ElevenLabs calls in the current milestone. Starter-pack MP3 playback depends on generated local files being present.
...
- `?cinematics=off|brief|full` controls optional orbit presentation and must default to `off` to preserve existing gameplay-camera behavior unless explicitly enabled.
...
- `?shotPreview=prePlayOrbit180|touchdownOrbit360` and `?cameraDebug=1` are development presentation tools only. They may expose shot progress, orbit center, radius, camera position, look target, and restore camera, but must not mutate gameplay state.
...
- `?cinematics=off`, `?cinematics=brief`, and `?cinematics=full` are supported, with `off` preserving existing camera behavior.
- `prePlayOrbit180` runs only during pre-snap setup, is computed from formation/field data, and never blocks or discards Space snap input.
- `touchdownOrbit360` starts only after an authoritative touchdown result, focuses the scorer/dead-ball context, is skippable, and does not alter reset timing or scoring.
- Orbit paths work from all snap lanes, resize with aspect ratio, avoid near-ground clipping, and restore the selected gameplay camera.
- `?shotPreview=prePlayOrbit180`, `?shotPreview=touchdownOrbit360`, and `?cameraDebug=1` expose shot debug information without mutating gameplay state.
- Audio and captions remain active while orbit shots run.
- Existing tactical, offense-perspective, cinematic broadcast, five-on-five gameplay, optional 7v7 passing drill, crowd preview, audio, route art, play cards, helmet, mannequin, visual, camera, unit, and browser smoke tests pass.
