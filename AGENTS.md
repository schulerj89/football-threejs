...
- Audio future scope: no runtime announcer playback, spatial player sounds, music playback, live text generation, conversational agents, second commentator, or browser-side ElevenLabs calls in the current milestone. Starter-pack MP3 playback depends on generated local files being present.
...
- The typed announcer script catalog includes the supported event categories with two or three caption-matched variants as required.
- The announcer identity is original, fictional, concise, and does not imitate named broadcasters or recognizable catchphrases.
- Announcer generation remains offline and Node-only; no ElevenLabs API code, key, request headers, or live generation path enters browser runtime code.
- When no configured announcer voice exists, the speech generator is prepared to preserve three voice-design previews, metadata sidecars, and a clearly labeled temporary prototype voice ID.
- Dry-run speech generation creates or refreshes the announcer caption manifest and audition page without making paid API requests.
- Execute-mode speech generation refuses to fabricate files when the API key is unavailable or rejected.
- Existing generated announcer files are not overwritten unless `--force` is supplied, and unchanged script/voice combinations are skipped.
- Announcer playback, live text generation, dynamic yard-number speech, second commentator behavior, camera changes, and gameplay changes are not implemented in this milestone.
