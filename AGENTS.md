...
- Roster future scope: active 11v11 gameplay is currently limited to the optional one-play Inside Zone 11 development drill. 11v11 passing, additional 11v11 plays, full special teams, additional roster tiers beyond the current default 7v7 playbook and legacy 5v5 regression mode, player switching, and broader formation families are deferred.
...
- Static 11v11 formation staging uses `ELEVEN_ON_ELEVEN_ROSTER` and `src/elevenOnElevenFormation.ts`. It preserves the shared 7v7 IDs where practical, adds tackles, tight end, slot, linebackers, and strong safety, and tracks football-position metadata separately from behavior-oriented `PlayerRole`.
- `?formationPreview=11v11` must remain a development-only static preview. It may resolve, validate, render, label, measure, and frame twenty-two players, but must not activate `?playbook=11v11`, run AI, snap into live play, or add 11v11 play cards.
- `?playbook=11v11` is an explicit optional development playbook with only Inside Zone 11. Keep 7v7 as the normal default until the user requests a default-mode promotion.
...
- `ELEVEN_ON_ELEVEN_ROSTER`, `ELEVEN_ON_ELEVEN_OFFENSE_PLAYER_IDS`, `ELEVEN_ON_ELEVEN_DEFENSE_PLAYER_IDS`, and `ELEVEN_ON_ELEVEN_PLAYER_IDS` exist while 7v7 remains the normal playable default.
- `src/elevenOnElevenFormation.ts` resolves all twenty-two positions from snap position, line of scrimmage, direction of play, field/boundary side, sideline insets, and named formation measurements without independent clamping.
- All three snap lanes validate, and the preferred-side mirror control resolves a legal mirrored formation.
- Validation proves exactly eleven players per team, unique expected IDs, no unexpected IDs, no overlaps, in-bounds placement, seven offensive players on the line, four in the backfield, correct eligibility metadata, defensive-side legality, corner alignment, and safety alignment from eligible receiving threats.
- `?formationPreview=11v11` renders all twenty-two current mannequins and helmets, keeps Space from starting a play, and frames the complete formation in tactical, offense-perspective, and cinematic camera modes.
- Formation audit/debug output includes ID, football position, line/backfield status, eligibility, distance from snap, and distance from line of scrimmage.
- Existing 7v7 gameplay behavior, 5v5 regression paths, build, unit tests, and browser smoke tests pass.
