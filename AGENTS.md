...
- The default URL still starts the existing five-on-five drill.
- `?playbook=7v7` starts the fourteen-player `Twin Slants Flat` passing drill.
- The 7v7 play has exactly three eligible receivers in stable order: left receiver, right receiver, running back.
- `E` cycles targets and `F` throws toward the selected target.
- The quarterback can scramble, lose forward-pass eligibility past the line, be sacked before release, throw incompletions, or complete passes through the existing deterministic pass arc.
- After a catch, possession and user control transfer to the receiver and defenders pursue the new carrier.
- The graphical play card is generated from the same 7v7 formation, route, protection, and coverage data used by gameplay.
- Existing five-on-five gameplay, 7v7 preview, helmet, visual, camera, unit, and browser smoke tests pass.
