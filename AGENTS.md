...
- Runtime commentary consumes immutable presentation events and never calls ElevenLabs or mutates gameplay state.
- Touchdown, turnover, sack, first-down, big-gain, incomplete, and informational commentary priorities are enforced, with touchdown suppressing queued first-down commentary when both are present.
- Announcer clips do not overlap, immediate variant repetition is avoided, per-category cooldowns work, and ordinary plays do not all trigger speech.
- Crowd ducking occurs only while speech is active and restores after completion, mute, reset, hidden-page suppression, or stopped speech.
- Optional captions display exact script text and remain hidden when captions or announcer playback are disabled.
- `?announcer=0`, `?captions=1`, and `?commentaryDebug=1` work without loading speech when audio or announcer playback is disabled.
- Announcer on/off, announcer volume, and captions on/off settings are available through runtime audio controls.
- Live text generation, dynamic names/statistics, second commentator behavior, camera changes, and gameplay changes are not implemented in this milestone.
