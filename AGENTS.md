...
- The title/setup screen exposes fictional user and opponent team selection, home/away uniforms, swatches, color pickers, reset-to-defaults, and deterministic similar-uniform warnings.
- Persisted team profile settings migrate invalid colors safely and apply through the versioned settings store.
- The resolved team theme drives player uniforms, helmet shell and faceguard colors, HUD/scorebug styling, play-card markers, field end zones, and supported crowd accent colors without gameplay changes.
- Skin tones remain independent from team colors, role colors remain debug-only, and material counts stay bounded through shared materials.
- README, AGENTS, package version, and CHANGELOG reflect the team-profile and uniform-customization release.
