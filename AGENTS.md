...
- `?crowdPreview=1` renders the field, simple procedural seating shell, and instanced low-poly spectators without enabling normal-game crowd presentation.
- `?crowdCount=500`, `2000`, `5000`, and `10000` are supported and safely clamped.
- Spectators use bounded shared geometries/materials and InstancedMesh batches; no per-spectator `Object3D`, geometry, or material allocation is introduced.
- Near/far visual detail tiers are active and deterministic.
- Preview camera controls cover wide, sideline, end-zone, and close crowd-inspection views.
- `?crowdBenchmark=1` produces a report for the standard count sweep with requested/actual counts, crowd draw calls, triangles, geometry/material/texture counts, estimated instance-buffer bytes, frame time, minimum observed FPS, and renderer memory counters.
- Crowd preview does not run gameplay AI, create active gameplay players, mutate gameplay state, or start a live play.
- Preview-owned GPU resources are disposed in tests without disposing globally shared gameplay resources.
- Existing five-on-five gameplay, optional 7v7 passing drill, audio, route art, play cards, helmet, mannequin, visual, camera, unit, and browser smoke tests pass.
