...
- Every receiver route begins at the receiver's resolved formation position and follows declared waypoints in order.
- Route distances, segment lengths, sampling, tangents, projection, and cross-track error are deterministic and unit tested.
- Route progress advances by supplied delta time and route speed, not frame count.
- Pre-snap receivers remain stationary; reset and play selection restore route progress to zero.
- Catch control transfer stops automatic route following immediately.
