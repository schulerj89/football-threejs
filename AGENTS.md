...
- Planned generated assets belong under `public/audio/sfx`, `public/audio/crowd`, and `public/audio/announcer`. Runtime playback and generated asset approval/import are future queued work.
...
- Official ElevenLabs `setup-api-key`, `sound-effects`, and `text-to-speech` skills are installed and inspected.
- Project-specific football audio director and broadcast writer skills exist under `.codex/skills`.
- Typed audio plan, dry-run SFX generation, dry-run speech generation, and audio reporting scripts exist under `tools/audio`.
- Audio generation defaults to dry-run; paid execution requires `--execute`.
- Missing-key, existing-file protection, invalid output path, dry-run, plan validation, and browser-secret scanning tests pass.
- `.env.example` is safe, local secret files are gitignored, and no browser source imports ElevenLabs or references `ELEVENLABS_API_KEY`.
- Runtime gameplay audio, announcer playback, crowd rendering, camera changes, and gameplay changes are not implemented in this milestone.
