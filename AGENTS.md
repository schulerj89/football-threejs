...
- Normal-game crowd presentation lives in `src/presentation/CrowdPresentationController.ts` and is opt-in through `?crowdVisuals=1` or persisted crowd presentation settings.
...
- `?crowdVisuals=1` enables the normal-game budgeted visual crowd without making the development-only `?crowdPreview=1` path run gameplay.
- `?crowdDensity=low|medium|high` maps to benchmarked 500, 2,000, and 5,000 spectator presets.
- Crowd reactions support idle, anticipation, first-down, touchdown, and disappointment states using deterministic instance subsets and bounded low-frequency matrix updates.
- First-down and touchdown visual reactions consume the same stable presentation event IDs used by audio/commentary and do not fire twice from camera transitions, reset delays, tab restoration, or UI rerenders.
- `?cinematics=off`, `?cinematics=brief`, and `?cinematics=full` are supported, with `off` preserving existing reset timing and gameplay-camera behavior.
- Full cinematics can use first-down and touchdown fan cutaways through the existing `PresentationCameraDirector`; touchdown cutaways chain to the scorer orbit after the authoritative touchdown result.
- Space/reset skip active shots and presentation holds without discarding gameplay input.
- `?presentationAudit=1` during normal gameplay exposes the presentation hardening matrix for audio, announcer, captions, crowd visuals, crowd reactions, cinematics, camera mode, active shot, holds, duplicate event suppression, frame time, draw calls, triangles, and audio memory counters.
- Disabled crowd visuals and disabled audio load no unnecessary crowd visual resources or generated audio assets.
- Existing tactical, offense-perspective, cinematic broadcast, five-on-five gameplay, optional 7v7 passing drill, crowd preview, audio, route art, play cards, helmet, mannequin, visual, camera, unit, build, and browser smoke tests pass.
