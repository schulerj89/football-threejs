...
- `src/config/GameExperienceSettings.ts` is the authoritative facade for normal-game presentation settings.
- A plain normal-game launch resolves to the broadcast preset without requiring URL parameters.
- The broadcast preset enables the 5v5 playbook, offense camera, brief cinematics, low-density crowd visuals, crowd reactions, audio, crowd audio, announcer, route art, and player motion while keeping captions off.
- The performance preset disables expensive visual presentation while keeping current gameplay behavior and player motion usable.
- Custom settings persist, and query overrides take precedence without modifying persisted settings.
- Development-only preview and audit modes remain opt-in.
- The debug readback snapshot reports effective preset, persisted settings, query overrides, final resolved settings, development-only mode flags, and audio/crowd asset readiness.
