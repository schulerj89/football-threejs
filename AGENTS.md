...
- On-field receiver route art is visible during pre-snap and hidden immediately during live and dead play unless route audit is enabled.
- On-field route art, graphical play cards, and receiver simulation consume the same resolved route points.
- Route art shows ordered paths, break points, start locations, route-end arrowheads, and selected-receiver highlighting.
- `?routeArt=0` disables normal route art; `?routeAudit=1` shows route ID, receiver ID, active segment, distance, completion percentage, nearest projected point, and cross-track error.
- Route-audit projection and tolerance math are deterministic and unit tested.
- Existing five-on-five gameplay, optional 7v7 passing drill, 7v7 preview, helmet, visual, camera, unit, and browser smoke tests pass.
