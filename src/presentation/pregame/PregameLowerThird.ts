...
export function syncPregameDebugOverlay(
  ...
  overlay.textContent = [
    ...
    `introOverlay: ${snapshot.introOverlay}`,
    `keys: ${snapshot.keysToGame.length}`,
    ...
    snapshot.quarterbackAppearance
      ? [
          `qbAppearance roster: ${snapshot.quarterbackAppearance.rosterPlayerId ?? 'none'}`,
          `bodyReady: ${snapshot.quarterbackAppearance.bodyReady ? 'yes' : 'no'}`,
          `helmetReady: ${snapshot.quarterbackAppearance.helmetReady ? 'yes' : 'no'}`,
          `helmetAsset: ${snapshot.quarterbackAppearance.helmetAssetId}`,
          `shellMaterial: ${snapshot.quarterbackAppearance.shellMaterialName ?? 'none'}`,
          `faceguardMaterial: ${snapshot.quarterbackAppearance.faceguardMaterialName ?? 'none'}`,
          `headBounds: ${snapshot.quarterbackAppearance.headBounds ? 'yes' : 'no'}`,
          `helmetBounds: ${snapshot.quarterbackAppearance.helmetBounds ? 'yes' : 'no'}`,
          `subjectVisible: ${snapshot.quarterbackAppearance.subjectVisible ? 'yes' : 'no'}`,
          `fallback: ${snapshot.quarterbackAppearance.fallbackReason ?? 'none'}`,
        ].join('\n')
      : 'qbAppearance: unavailable',
